...
import com.banking.auth.customRequest.CustomRequestForResetPassword;
...
public class CustomerAuthenticationController {
	...
	public ResponseEntity<Object> updateCustomer(@RequestBody Customers customer){
		...
		Customers custAccunt = customerService.fetchCustomerId(customer.getCustomerId());
		if(custAccunt != null) {
			
			custAccunt.setFirstName(customer.getFirstName());
			custAccunt.setMiddletName(customer.getMiddletName());
			custAccunt.setLastName(customer.getLastName());
			custAccunt.setMobileNumber(customer.getMobileNumber());
			custAccunt.setAddress(customer.getAddress());
	...
	public ResponseEntity<Object> ResetPassword(@RequestBody CustomRequestForResetPassword customer){
		...
		Customers fetchmail = customerService.findCustomerByMail(customer.getEmailId());
		if(fetchmail!= null) {
			if(customer.getOldPassword().equals(customer.getNewPassword())){
				if(passwordEncoder.matches(customer.getNewPassword(),fetchmail.getPassword())) {
					return new ResponseEntity<Object> ("oldPassword" ,HttpStatus.OK);
				}
				else {
					int n =customerService.ResetPassword(customer.getNewPassword(),customer.getEmailId());
				 }}
			else {
		...
		return new ResponseEntity<Object>("password reset sucessfully ",HttpStatus.OK);
	}
